 ...
 *
 */
public class WriteAccessConfig implements Runnable {
	private static final Logger logger = LoggerFactory.getLogger(WriteAccessConfig.class);
	...
	public WriteAccessConfig(){
		super();
	}
	
	public WriteAccessConfig(String url, JSONObject param, String method, String authorization, DeviceAutomationService deviceAutomationService){
		this.url = url;
		this.param = param;
		this.method = method;
	...
	public void run() {
		try{
			...
			String info = "写入接入交换机配置成功";
			boolean success = true;
			if(StringUtils.isEmpty(sb)){
				info = "调用写入接入交换机配置接口返回空信息";
				success = false;
				logger.error(info);
			}else{
